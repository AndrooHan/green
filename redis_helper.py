...
feed_posts = {
    "71e5cfa0-9e18-4810-8f73-afdb009203c0": {
    ...
    "6fb4a31e-c9fa-4ab3-b160-9e043011426c": {
    ...
    "4efa3744-0d5c-47d9-aa18-5c922a45c899": {
...
class Redis:
   ...
   def __init__(self):
      ...
      if Redis.__instance != None:
        ...
        is_local = str(ip_address) == "127.0.0.1"
        Redis.__instance = fakeredis.FakeStrictRedis() if is_local else redis.from_url(os.environ.get("REDIS_URL"))

# Redis functions ===============================
def seed_redis():
    Redis.get().flushdb()
    for pid, post in feed_posts.items():  
        json_post = json.dumps(post)
        Redis.get().set(pid, json_post)
